...
class Kafka {
  ...
  * start() {
  ...
  // function* buildProtoBuf(eventName, message) {
  // // TODO: Replace the hardcoded file path dynamically
  // // by mapping it to a config using unique_EventName: File, messageDef1
  // var protoRoot = yield protobuf.load('../../../../protos/test/test.proto').then(function (root){
  // return root;
  // }).catch(function (err) {
  //   console.log(err);
  // });
  // var root = protoRoot;
  // var TestRequest = root.lookup("test.TestEvent");
  // var buffer = TestRequest.encode(message).finish();
  // return buffer;
  // }
  ...
  * $send(topicName, eventName, message, messageType) {
    ...
    try {
      ...
      for (let i = 0; i < messages.length; i += 1) {
        ...
        const bufferObj = yield co(function* createBufferObject(eventName,
        message) {
          // by mapping it to a config using unique_EventName: File, messageDef1
          var protoRoot = yield protobuf.load('../../../../protos/test/test.proto').then(function (root){
          return root;
          ...
          var root = protoRoot;
          var TestEvent = root.lookup("test.TestEvent");

          var inputNew = { value: "value", count: 1};
          var intermediateMessage = TestEvent.create(inputNew);

          var buffer = TestEvent.encode(intermediateMessage).finish();
          return buffer;
    });
